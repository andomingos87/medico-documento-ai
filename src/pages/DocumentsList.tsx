      ...
      <div className="bg-white rounded-lg border border-neutral-200 shadow-card p-4">
        <div className="flex flex-col md:flex-row gap-4 mb-6">
          <div className="flex-1 relative">
            ...
            <Input placeholder="Buscar por paciente ou procedimento..." className="pl-9" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
          </div>
        ...
        {filteredDocuments.length > 0 ? <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {filteredDocuments.map(doc => <DocumentCard key={doc.id} id={doc.id} title={doc.title} status={doc.status} createdAt={doc.createdAt} documentType={doc.documentType} onView={handleViewDocument} onEdit={handleEditDocument} onDelete={handleDeleteDocument} onSign={handleSignDocument} />)}
